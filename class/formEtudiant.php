...
<link rel="stylesheet" type="text/css" media="screen" href="../menu/style.css">
   ...
   //include_once '../menu/menu.php';
    ...
    Autoloader::register();
     ...
     $bat=$connexion->query('SELECT * FROM batiment');
     $cham=$connexion->query('SELECT * FROM chambre');
    $champarbat=array();
     foreach($cham as $donnees){
      $champarbat[$donnees->id_chambre]=$donnees->nom_chambre;
var_dump( $champarbat);
     }
...
<div class="col-md-6" id="ad">
 <label for ="adresse">Adresse</label>
  ...
  <div class="col-md-6" id="bat">
             <label for ="chambre">Batiment</label>
             <?php
  require_once 'Autoloader.php';
  Autoloader::register();
   $connexion=Database::connect();
...
<div class="col-md-6" id="ch">
  ...
  Autoloader::register();
   $connexion=Database::connect();
   while ($champarbat = $cham->fetch()){
    ?>
  <SELECT class="form-control"  name="chambre">
  <?php foreach($cham as $id =>$name): ?>
